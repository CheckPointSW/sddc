...
"""
Intances should be tagged (AWS) or have metadata (OpenStack) in order to
identify whether they should be synchronized and what template should be used:

Instance tags (metadata):

x-chkp-management: 'VALUE-SHOULD-MATCH-THE-MANAGEMENT-CONFIGURATION-NAME'
x-chkp-template: 'TEMPLATE-NAME'


In AWS:

Instances can optionally have this tag:

x-chkp-ip-address: 'IP-ADDRESS'|private|public

Network interface objects (eni) can optionally have the following tags:

x-chkp-topology: external|internal
x-chkp-anti-spoofing: true|false


In OpenStack, gateways with more than one interface must have this tag:

x-chkp-interfaces: 'NET-NAME-FOR-eth0:NET-NAME-FOR-eth1:...'


On the management:

Tags are used on gateway objects to reflect their origin and provisioning
state.
The names of the gateways are prefixed with the controller name.
...

{
    "delay": 'INT',
    "management": {
        "name": "'VALUE-OF-x-chkp-management-TAG'",
        "host": "'IP-ADDRESS-OR-HOST-NAME[:PORT]'",
        "fingerprint": "'DIGEST-ALGORITHM':'FINGERPRIT-IN-HEX'",
        "user": "'STRING'",
        "b64password": "'BASE64-ENCODED-STRING'",  //  Either "b64password" or
        "password": "'STRING'",                    //  "password"
        "surrogates": ["'IP-ADDRESS'", ...]        //  Optional: dev only
    },
    "templates": {
        "'TEMPLATE-NAME'": {
            "proto": "'PARENT-TEMPLATE-NAME'",
            "policy": "'STRING'",
    ...
    "controllers": {
        "'UNIQUE-NAME'": {
            ...
            "access-key": "'STRING'",
            "secret-key": "'STRING'",
            "session-token": "'STRING'",  // optional
            "cred-file": "'ALTERNATIVE-TO-THE-3-ABOVE'",
            "regions": ["'AWS-REGION'", ...]
        },
        "'UNIQUE-NAME'": {
            "class": "OpenStack",
            ...
            "fingerprint": "'DIGEST-ALGORITHM':'FINGERPRIT-IN-HEX'",
            "user: "'STRING'",
            "b64password": "'BASE64-ENCODED-STRING'",  //  Either
                                                       //  "b64password" or
            "password": "'STRING'",                    //  "password"
            "tenant": "'TENANT-UUID'"
        },
